...
static int w=0;  
//Questa variabile static serve a tenere conto di quante volte si usa distruggi terra
static int dt=1;                             
//Inizializza la struttura Oberon con 10 oro, 5 punti ferita, 2 incantesimi e 1 distruggi terra
static struct Oberon Oberon = {10, 5, 2, 1};  

//Questa funziona chiude il percorso impedendo altre modifiche
void chiudi_percorso(){

	//Se non è stata aggiunta nessuna terra non si può chiudere il percorso e ritorna al sottomenù per gestire il percorso
	if(head==NULL){
...
//Questa funzione apre un sottomenù che richiama attraverso uno switch altre funzioni per la modifica del percorso
void crea_percorso(){  
  ...
  //Stampa il sottomenù finche non si decide di tornare indietro impostando t=5                            
    ...
    case 3:{
      //Chiama la funzione per stampare su schermo le terre finora inserite
      stampa_percorso();               
      break;
    }
    case 4:{
...
void ins_terra(){          
   //Queste variabili servono ad impostare il tipo di terra (tt) e il tipo di mostro (tm), x serve a gestire i cicli e vieni impostata a  0 ogniqualvolta si chiama la funzione
  int tt, tm,x=0; 
         
  //Questa variabile imposta il tipo di tesoro (ts), cioè la quantità d'oro presente nella terra
  short int ts;	 
           
  //Controlla che il percorso non sia stato chiuso. Se w fosse diverso da 0 il percorso sarebbe già stato chiuso e quindi non si può inserire la terra
  if(w==0){	           
	...
	do{
	  ...
	  //Controlla se si è inserito uno scheletro in una palude: ciò non deve accadere e pertanto si deve scegliere un altro mostro
	  else if(tm==1 && tt==2){
    ...
    do{
	 ...
	 printf("Il tesoro è troppo grande!\n"); 
       ...
       //Controlla che il tesoro non sia minore di 0
       else if(ts<0){
	 printf("Il tesoro non può essere minore a 0!\n");
     }
       //Controlla che il tesoro non sia maggiore di 200
       else if(ts>200){
	 printf("Il tesoro è troppo grande!\n");
     }
    ...
    //Controlla che la terra inserita sia la prima. In caso affermativo, si alloca memoria nella struttura head e si impostano i valori di questa secondo i valori precedentemente inseriti
    if(head==NULL){
...
//Se il percorso è stato chiuso (w è diverso da 0), non si possono aggiungere terre
  else{  
...
void stampa_percorso(){
 ...
 //Questo puntatore serve per scorrere la lista di tutte le terre create finora cominciando dalla prima
  ...
  //Se si è inserita almeno una terra, la funziona continua normalmente
    ...
    //c è un contatore: aumenta ad ogni ciclo di uno (e quindi conta quante terre si sono inserite) e comincia da 1
    int c=1;
	...
	break;
      ...
      //Controlla quale mostro è presente nella terra
    ...
    }while(pp != NULL); //ripete il ciclo finchè non arriva in fondo alla lista
  } 
}
...
//Questa funzione libera dalla memoria tutte le terre inserite ed esce dal programma
void termina_gioco(){
...
void muovi_Oberon(){ 
  ...
  //Se è la prima volta che si usa la funzione, px viene posto = head, per iniziare dalla cima della lista (solo la prima volta)
  else{
	static int c=0;
	if(c==0){
	  px = head;
	...
	do{
	  //Stampa un sottemenù finchè non si avanza dalla terra, trovandoci in un villaggio non abbiamo l'opzione per combattere ma quella per visitare il negozio
...
void negozio(){
  ...
  //Il menu viene stampato finche non si esce dal negozio
  do{	                                                                        
    
	//Evento segreto: se ci si presenta al negozio con esattamente 42 oro si avrà una nuova opzione (Ispirato al libro "Guida Galattica per Autostoppisti") Opzioni di negozio normale più in basso
	...
	//Se si sceglie l'opzione bonus il gioco finisce
      case 3:{
	printf("Ti teletrasporti davanti al tuo castello.\nIl tuo maggiordomo Marvin ti accoglie dicendoti che il tuo castello verrà distrutto per far posto a un'autostrada galattica.\n");
	termina_gioco();
	break;
      }
	//Opzione per uscire dal negozio
      case 4:{
	muovi_Oberon();
	break;
      }
	...
	//Opzioni per il negozio normale
    else{
      printf("\nBenvenuto al mercato nero dei goblin del villaggio.\n");
      printf("Cosa vuoi acquistare?\n");
      printf("1) Incantesimo, costa 120  2) Pozione, costa 90. 3)Esci dal negozio\n");
      //Lo switch gestisce le scelte
      scanf("%d", &r);
      
      switch(r){
	
      case 1:{
	//A seconda della quantità di oro in possesso vengono stampate frasi diverse
	if(80<Oberon.borsa_oro&&Oberon.borsa_oro<120){
	...
	//Se l'oro è sufficiente la quantità viene sottratta e si aggiunge un incantesimo
	if(Oberon.borsa_oro>=120){
	...
	break;}
      ...
      case 2:{
	//Come sopra, vengono stampate frasi diverse a seconda dell'oro che si ha
	if(60<Oberon.borsa_oro&&Oberon.borsa_oro<90){
	...
	printf("Vedo che sei al verde... Non che mi aspettassi altro, da un viandante come te.\n");
      ...
      //Se il denaro è sufficiente viene sottratta la quantità e si aggiunge una pozione 
      if(Oberon.borsa_oro>=90){
...
void avanza(){
  ...
  //Se la terra in cui si trova è l'ultima della lista, Oberon ha vinto e si termina l'esecuzione del programma
    ...
    printf("Tornato a casa, puoi finalmente goderti un bicchiere di ippocrasso.\n");
    printf("Gurdando nella tua borsa, ti sei accorto che nella tua avventura hai racimolato %d oro.\n", Oberon.borsa_oro);
    termina_gioco();
  }
...
//Questa funzione serve ad usare la pozione per ristabilre a 5 i punti ferita
void usa_pozione(){
  //Controlla che Oberon.pozione_guaritrice sia almeno 1, dopodichè sottrae 1 a quella quantità e reimposta a 5 Oberon.punti_ferita
...
void usa_incantesimo(){
  ...
  //Se si hanno incantesimi disponibili, si sottrae 1 alla valore di Oberon.incantesimi e si setta il Tipo_mostro a 0 (cioè nessuno)
  else{
    Oberon.incantesimi-=1;
    printf("Abracadabra, OcusPocus! Bibidi, Bodibi, Bu! Insomma, il mostro si dissolve in un mucchietto di ceneri\n");
    px->Tipo_mostro=0;
  }
}
...
void distruggi_terra(){
  ...
  //Se Oberon si trova sull'ultima Terra, distrugge la sua dimora, perdendo la partita
    ...
    if(px->next==NULL){
      printf("Hai distrutto la tua dimora. Piangi sconsolato mentre guardi la tua riserva di ippocrasso scorrere via.\n");
      termina_gioco();
    }
...
//Questa funzione gestisce lo scambio di danni tra Oberon e il mostro attraverso la generazione di numeri casuali
int attacca(int pfa, int pf){
  ...
  //Se il numero generato è maggiore o uguale a 60, l'attacco va a segno e si sottraggono quindi 3 punti dai punti ferita attuali del mostro
  if(atk>=60){
  ...
  //Se il mostro sopravvive, si genera un numero casuale tra 0 e 99 per l'attacco del mostro
    ...
    atkm = rand()%100;
    //Se maggiore o uguale a 50, il mostro colpisce e infligge danni pari ai suoi punti farite massimi
    if(atkm>=50){		
    ...
    //Se Oberon muore, la partita termina con la sconfitta di Oberon e il programma viene arrestato
